...
def flesch_port(pylinguistObj):
    ILF = 248.835 - (1.015 * float(pylinguistObj.avg_word_per_sentence)) - (
                84.6 * float(pylinguistObj.avg_syllables_per_word))
...
def calcAmbiquity(pylinguistObj):
    ...
    if (nVerb + nNoun + nAdjective + nAdverb) > 0: AvgAmbiquity = (mVerb + mNoun + mAdjective + mAdverb) / float(
        nVerb + nNoun + nAdjective + nAdverb)
    return AvgAmbiquity
